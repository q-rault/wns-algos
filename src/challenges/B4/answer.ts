...
export default function ({ submissions }: { submissions: Submission[] }): any {
  const subsWithMonth = submissions.map((submission) => {
    const submissionDate = new Date(submission.submittedAt);
    const submissionMonth = new Date(
      Date.UTC(submissionDate.getUTCFullYear(), submissionDate.getUTCMonth(), 1)
    ).toISOString();
    return { ...submission, month: submissionMonth };
  });

  const sortedSubsWithMonth = subsWithMonth.sort((a, b) =>
    a.submittedAt.localeCompare(b.submittedAt)
  );

  const subsByMonth = sortedSubsWithMonth.reduce<MonthSubmission[]>(
    (acc: MonthSubmission[], subWithMonth) => {
      const { month, ...sub } = subWithMonth;
      const monthIndex = acc.findIndex((item) => item.month === month);

      if (monthIndex >= 0) {
        acc[monthIndex].submissions = [
          ...acc[monthIndex].submissions,
          sub,
        ];
        return acc;
      }
      return [...acc, { month, submissions: [sub] }];
    },
    []
  );
  return subsByMonth;
